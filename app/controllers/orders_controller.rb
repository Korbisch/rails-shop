class OrdersController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @order.update(order_params)
        OrderMailer.shipped(@order).deliver_later if order_params[:ship_date]
  ...
  def pay_type_params
    if order_params[:pay_type] == "Credit card"
      order_params.permit(:credit_card_number, :expiration_date)
    elsif order_params[:pay_type] == "Check"
      order_params.permit(:routing_number, :account_number)
    elsif order_params[:pay_type] == "Purchase order"
      order_params.permit(:po_number)
    else
      {}
    end
  end
